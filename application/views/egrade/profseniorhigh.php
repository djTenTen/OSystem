...
<h1>Senior High Teachers</h1>
        ...
        <div style="overflow-y:scroll;width:100%;height:400px">        
            ...
            <tbody id="myTable">
                <?php foreach($seniorhighProf as $row){?> 
                    
                    <tr>
                        <td><?= $row['Teacher'];?></td>
                        <td><?= $row['FullName'];?></td>
                        <td>
                            <a class="btn btn-outline-primary btn-sm" href="viewclasssubjectsshs/<?= $row['Teacher'];?>" data-toggle="tooltip" title="View Subject list"><span class="fas fa-tasks"></span></a>
                        </td>
                    </tr>
                <?php }?>
            </tbody>
